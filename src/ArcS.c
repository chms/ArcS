...
#define ARCS_PLUS              true
#define ARCS_CS                false
...
// Color schemes
#define COLOR_SCHEME_APLITE_DARK
#define COLOR_SCHEME_APLITE_LIGHT
...
static void update_layer_arcs(Layer *layer, GContext *ctx) {
    #if FAKE_TIME
    time_t now = 419903100;
    #else
    ...
    graphics_draw_arc(ctx, window_center, window_center.x, HOUR_CIRCLE_THICKNESS, angle_270, hour_angle+angle_270, Color1);
    graphics_draw_arc(ctx, window_center, window_center.x-CIRCLE_SPACING_THICKNESS-HOUR_CIRCLE_THICKNESS, MINUTE_CIRCLE_THICKNESS, angle_270, minute_angle+angle_270, Color1);
}

static void update_layer_battery_bluetooth(Layer *layer, GContext *ctx) {
    ...
    uint8_t charge_percent = battery_state.charge_percent;
    uint8_t x_bar = shift_battery + 2;
    
    // Fill battery bars
    while(charge_percent > 10) {
        graphics_draw_line(ctx, GPoint(x_bar,5), GPoint(x_bar, 8));
        x_bar++;
        charge_percent = charge_percent - 9;
    }
}
...
static void show_digital_clock(bool show_digital_clock) {
    ...
    layer_set_hidden(bitmap_layer_get_layer(bitmap_layer_logo), digital_clock_is_visible || !ARCS_CS);
...
static void battery_handler(BatteryChargeState charge) {
    layer_mark_dirty(layer_battery_bluetooth);
}
...
static void window_load(Window *window) {
    ...
    window_set_background_color(window, Color0);
    ...
    text_layer_set_text_color(text_layer_time, Color1);
    ...
    text_layer_set_text_color(text_layer_date, Color1);

    // Create and style arc (analog) layer
    layer_arcs = layer_create(window_bounds);
    layer_set_update_proc(layer_arcs, update_layer_arcs);

    // Create and style battery layer
    layer_battery_bluetooth = layer_create(GRect(58, 44, 28, 13));
    layer_set_update_proc(layer_battery_bluetooth, update_layer_battery_bluetooth);
